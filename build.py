...
with open("src\\tcpgecko\\tcp_gecko.cpp") as f:
    s=f.read()
    f.close()

sa=s[s.index("// Start MamiesMod Client")+26:s.index("			// End MamiesMod Client")]
s=s.replace(sa,code)

with open("src\\tcpgecko\\tcp_gecko.cpp",'w') as f:
    f.write(s)
    f.close()
